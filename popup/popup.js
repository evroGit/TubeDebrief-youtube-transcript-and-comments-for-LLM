...
[contentSourceInput, minCharsInput, maxCommentsInput, includeRepliesInput].forEach((el) => {
...
async function handleOpen(targetKey) {
  const [activeTab] = await chrome.tabs.query({ active: true, currentWindow: true });
  const result = await openLLMTarget(targetKey, activeTab?.url);
  if (!result.ok && result.reason === 'no-text') {
    ...
    const message = t(currentLang, 'statusErrorGeneric');
    setStatus(message, true);
    await saveLastError(message);
  } else {
...
document.querySelectorAll('.open-targets button').forEach((button) => {
  button.addEventListener('click', () => handleOpen(button.dataset.target));
});
