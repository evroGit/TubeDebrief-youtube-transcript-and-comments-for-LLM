...
Copy и Open — независимые действия. Это специально: раньше была одна кнопка "Copy and Open", привязанная к одному заранее выбранному LLM, из-за чего для каждого нового LLM приходилось заново гонять весь сбор комментариев.
...
*(Если в будущем понадобится YouTube Data API: потребуется завести проект в Google Cloud Console, включить YouTube Data API v3 и создать API key — это отдельная лишняя настройка, которую MVP сознательно не требует.)*
...
| `promptTemplate` | options | редактируемый текст инструкции для LLM (плейсхолдеры `{{videoTitle}}`, `{{videoUrl}}`, `{{count}}`); список комментариев добавляется после него автоматически |
